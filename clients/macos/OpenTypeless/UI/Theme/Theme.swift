...
@MainActor
final class OpenTypelessThemeController: ObservableObject {
    static let shared = OpenTypelessThemeController()
    ...
    private var currentMode: OpenTypelessThemeMode {
        let rawValue = UserDefaults.standard.string(forKey: OpenTypelessThemeStorageKeys.themeMode)
        return OpenTypelessThemeMode(rawValue: rawValue ?? "") ?? .system
    }
}

private struct ThemeRefreshModifier: ViewModifier {
    @ObservedObject private var theme = OpenTypelessThemeController.shared
...
enum AppColors {
    ...
    private static func resolvedPalette(for isDark: Bool) -> ResolvedPalette {
        let variant: OpenTypelessThemeVariant = isDark ? .dark : .light
        let storageKey = isDark ? OpenTypelessThemeStorageKeys.darkThemePresetID : OpenTypelessThemeStorageKeys.lightThemePresetID
        let presetID = UserDefaults.standard.string(forKey: storageKey)
        let profile = OpenTypelessThemePresetCatalog.profile(for: presetID, variant: variant)
...
private struct ResolvedPalette {
    ...
    init(profile: OpenTypelessThemeProfile, isDark: Bool) {
        let background = NSColor(openTypelessHex: profile.backgroundHex)
            ?? (isDark ? NSColor(red: 0.08, green: 0.08, blue: 0.09, alpha: 1) : NSColor(red: 0.97, green: 0.96, blue: 0.94, alpha: 1))
        let foreground = NSColor(openTypelessHex: profile.foregroundHex)
            ?? (isDark ? NSColor.white : NSColor.black)
        let accentBase = NSColor(openTypelessHex: profile.accentHex) ?? .systemOrange
        let successBase = NSColor(openTypelessHex: profile.successHex) ?? .systemGreen
        let warningBase = NSColor(openTypelessHex: profile.warningHex) ?? .systemOrange
        let dangerBase = NSColor(openTypelessHex: profile.dangerHex) ?? .systemRed
        let processingBase = NSColor(openTypelessHex: profile.processingHex) ?? .systemBlue
...
extension NSColor {
    convenience init?(openTypelessHex: String) {
        let cleaned = openTypelessHex
