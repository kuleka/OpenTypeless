...
struct ThemeSettingsView: View {
    ...
    private var modeBinding: Binding<OpenTypelessThemeMode> {
        Binding(
    ...
    private var modeCard: some View {
        SettingsCard(
            ...
            detail: localized("Choose how OpenTypeless follows system appearance, then mix curated light and dark palettes.", locale: locale),
            ...
            VStack(alignment: .leading, spacing: AppTheme.Spacing.lg) {
                Picker(localized("Theme mode", locale: locale), selection: modeBinding) {
                    ForEach(OpenTypelessThemeMode.allCases) { mode in
    ...
    private func presetCard(variant: OpenTypelessThemeVariant) -> some View {
        ...
        return SettingsCard(
            ...
            LazyVGrid(columns: columns, alignment: .leading, spacing: AppTheme.Spacing.md) {
                ForEach(OpenTypelessThemePresetCatalog.presets) { preset in
                    ThemePresetTile(
                        preset: preset,
                        variant: variant,
    ...
    private func selectedPresetID(for variant: OpenTypelessThemeVariant) -> String {
        switch variant {
        case .light:
...
private struct ThemePreviewPane: View {
    let title: String
    let preset: OpenTypelessThemePreset
    let variant: OpenTypelessThemeVariant
    let isActive: Bool

    private var profile: OpenTypelessThemeProfile {
        preset.profile(for: variant)
    }

    var body: some View {
        let background = Color(nsColor: NSColor(openTypelessHex: profile.backgroundHex) ?? .windowBackgroundColor)
        let foreground = Color(nsColor: NSColor(openTypelessHex: profile.foregroundHex) ?? .labelColor)
        let accent = Color(nsColor: NSColor(openTypelessHex: profile.accentHex) ?? .controlAccentColor)
            ...
            VStack(alignment: .leading, spacing: AppTheme.Spacing.sm) {
                HStack(spacing: AppTheme.Spacing.sm) {
                    Text(preset.badgeText)
                        .font(AppTypography.caption)
                        .foregroundStyle(Color(nsColor: NSColor(openTypelessHex: preset.badgeForegroundHex) ?? .labelColor))
                        .padding(.horizontal, 8)
                        .padding(.vertical, 5)
                        .background(
                            Color(nsColor: NSColor(openTypelessHex: preset.badgeBackgroundHex) ?? .windowBackgroundColor),
                            in: Capsule()
                        )
