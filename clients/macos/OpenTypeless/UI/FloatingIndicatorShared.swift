...
extension NSScreen {
    ...
    var openTypeless_displayNumber: UInt32 {
        (deviceDescription[NSDeviceDescriptionKey("NSScreenNumber")] as? NSNumber)?.uint32Value ?? 0
    }

    /// Whether two screens refer to the same physical display.
    func openTypeless_isSameDisplay(as other: NSScreen?) -> Bool {
        guard let other else { return false }
        let a = openTypeless_displayNumber
        let b = other.openTypeless_displayNumber
        if a != 0, b != 0 {
            return a == b
        }
        return self === other
...
extension Timer {
    ...
    @MainActor
    static func openTypeless_scheduleRepeating(interval: TimeInterval, block: @escaping (Timer) -> Void) -> Timer {
