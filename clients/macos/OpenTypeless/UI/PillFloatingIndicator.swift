...
final class PillFloatingIndicatorController: NSObject, ObservableObject, NSMenuDelegate, FloatingIndicatorPresenting {
    ...
    private func startHoverIntentMonitoring() {
        ...
        hoverIntentTimer = Timer.openTypeless_scheduleRepeating(interval: LayoutMetrics.hoverMonitorInterval) { [weak self] _ in
            Task { @MainActor in
    ...
    private func makeContextMenu() -> NSMenu {
        ...
        let menu = NSMenu(title: localized("OpenTypeless Pill", locale: locale))
        menu.delegate = self
    ...
    private func checkAndUpdateScreenPosition() {
        ...
        if let last = lastScreen, currentScreen.openTypeless_isSameDisplay(as: last) {
            return
        }
