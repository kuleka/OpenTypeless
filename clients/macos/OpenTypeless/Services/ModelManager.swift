...
class ModelManager {
    ...
    private func downloadWhisperKitModel(named modelName: String, onProgress: ((Double) -> Void)? = nil) async throws {
        ...
        do {
            ...
            Log.boot.info(
                "WhisperKit pipeline begin variant=\(modelName) storageLeaf=OpenTypeless/models/argmaxinc/whisperkit-coreml (under Application Support) uiProgressNote=0-80pct is file download 85-100pct is prewarm"
            )
