...
struct MentionRewriteServiceTests {
    private func makeSUT() -> (
        ...
        mockFS.filesByRoot = [
            "/workspace": [
                "/workspace/OpenTypeless/Services/AppCoordinator.swift",
                "/workspace/OpenTypeless/Services/AudioRecorder.swift",
                "/workspace/OpenTypeless/Services/TranscriptionService.swift",
                "/workspace/OpenTypeless/UI/Settings/SettingsWindow.swift",
                "/workspace/OpenTypeless/UI/Main/MainWindow.swift",
                "/workspace/OpenTypeless/Utils/Logger.swift",
                "/workspace/OpenTypeless/Models/TranscriptionRecord.swift",
                "/workspace/OpenTypelessTests/AudioRecorderTests.swift",
                "/workspace/README.md",
            ]
        ]
    ...
    @Test func deriveWorkspaceInsightsIncludesActiveDocumentAndTags() async {
        ...
        let insights = await fixture.sut.deriveWorkspaceInsights(
            workspaceRoots: ["/workspace"],
            activeDocumentPath: "/workspace/OpenTypeless/Services/AppCoordinator.swift"
        )

        #expect(insights.activeDocumentRelativePath == "OpenTypeless/Services/AppCoordinator.swift")
        ...
        #expect(insights.fileTagCandidates.first == "OpenTypeless/Services/AppCoordinator.swift")
    }
    ...
    @Test func vsCodeFormatsExactMentionPath() async {
        ...
        #expect(result.didRewrite)
        #expect(result.text.contains("@OpenTypeless/Services/AppCoordinator.swift"))
    }
    ...
    @Test func normalizeFilePathToParentDirectory() {
        ...
        mockFS.directories.insert("/workspace/OpenTypeless/Services")
        ...
        #expect(result == ["/workspace/OpenTypeless/Services"])
    }

    @Test func normalizeTildePath() {
        let fixture = makeSUT()
        let home = NSHomeDirectory()
        let expandedPath = "\(home)/Projects/opentypeless"
        var mockFS = fixture.mockFS
        mockFS.directories.insert(expandedPath)
        let sut = MentionRewriteService(fileSystem: mockFS)

        let result = sut.normalizeWorkspaceRoots(["~/Projects/opentypeless"])
    ...
    @Test func normalizeClimbsToProjectMarker() {
        ...
        var mockFS = fixture.mockFS
        mockFS.directories.insert("/workspace/OpenTypeless/Services")
        mockFS.directories.insert("/workspace/OpenTypeless")
    ...
    @Test func normalizeWithFileURLAndTildeAndFilePath() {
        let fixture = makeSUT()
        let home = NSHomeDirectory()
        let projectRoot = "\(home)/Projects/opentypeless"
        var mockFS = fixture.mockFS
        mockFS.directories.insert(projectRoot)
        mockFS.directories.insert("\(projectRoot)/OpenTypeless")
        mockFS.directories.insert("\(projectRoot)/.git")
        let sut = MentionRewriteService(fileSystem: mockFS)

        let result = sut.normalizeWorkspaceRoots([
            "file://\(projectRoot)/OpenTypeless/AppCoordinator.swift"
        ])
    ...
    @Test func rewriteWorksWithFilePathAsWorkspaceRoot() async {
        ...
        let result = await fileSut.rewrite(
            text: "check AppCoordinator.swift",
            capabilities: fixture.cursorCapabilities,
            workspaceRoots: ["/workspace/OpenTypeless/Services/AppCoordinator.swift"]
        )
    ...
    @Test func antigravityAdapterRewriteWithTildeRedactedRoot() async {
        ...
        mockFS.directories.insert(projectRoot)
        mockFS.directories.insert("\(projectRoot)/OpenTypeless")
        mockFS.directories.insert("\(projectRoot)/OpenTypeless/Services")
        mockFS.directories.insert("\(projectRoot)/.git")
        mockFS.filesByRoot[projectRoot] = [
            "\(projectRoot)/OpenTypeless/Services/AppCoordinator.swift",
            "\(projectRoot)/OpenTypeless/Services/AudioRecorder.swift",
        ...
        let result = await sut.rewrite(
            ...
            workspaceRoots: ["~/Projects/opentypeless"]
        )
    ...
    @Test func normalizeTildeRedactedFilePath() {
        ...
        let result = sut.normalizeWorkspaceRoots([
            "~/Projects/opentypeless/OpenTypeless/AppCoordinator.swift"
        ])
    ...
    @Test func routingSignalDerivesWorkspaceRootFromDocumentPath() {
        let snapshot = ContextSnapshot(
            ...
            appContext: AppContextInfo(
                ...
                selectedText: nil,
                documentPath: "~/Projects/opentypeless/OpenTypeless/AppCoordinator.swift",
                browserURL: nil
            ),
            clipboardText: nil,
            warnings: []
        ...
        #expect(signal.workspacePath == "~/Projects/opentypeless/OpenTypeless")
    }
    ...
    @Test func routingSignalDerivesWorkspaceRootFromFileURL() {
        let snapshot = ContextSnapshot(
            ...
            appContext: AppContextInfo(
                ...
                focusedElementValue: nil,
                selectedText: nil,
                documentPath: "file:///Users/test/Projects/opentypeless/file.swift",
                browserURL: nil
            ),
            clipboardText: nil,
            warnings: []
        ...
        #expect(signal.workspacePath == "/Users/test/Projects/opentypeless")
    }
    ...
    @Test func routingSignalUsesDirectoryDocumentPathForTerminalApps() {
        let snapshot = ContextSnapshot(
            ...
            appContext: AppContextInfo(
                ...
                focusedElementValue: nil,
                selectedText: nil,
                documentPath: "~/Projects/opentypeless/",
                browserURL: nil
            ),
            clipboardText: nil,
            warnings: []
        ...
        #expect(signal.workspacePath == "~/Projects/opentypeless")
    }
    ...
    private func makeEditorIntegrationSut() -> MentionRewriteService {
        ...
        let home = NSHomeDirectory()
        let root = "\(home)/Projects/opentypeless"
        fs.directories = [
            root,
            "\(root)/.git",
            "\(root)/OpenTypeless",
            "\(root)/OpenTypeless/Services",
        ]
        fs.filesByRoot = [
            root: [
                "\(root)/OpenTypeless/Services/AppCoordinator.swift",
                "\(root)/OpenTypeless/Services/AudioRecorder.swift",
                "\(root)/README.md",
            ]
        ]
    ...
    @Test func cursorDerivationAndRewriteFromFilePath() async {
        ...
        let result = await sut.rewrite(
            text: "open AppCoordinator.swift",
            capabilities: CursorAdapter().capabilities,
            workspaceRoots: ["\(home)/Projects/opentypeless/OpenTypeless/Services/AppCoordinator.swift"]
        )
    ...
    @Test func vsCodeDerivationAndRewriteFromFileURL() async {
        ...
        let result = await sut.rewrite(
            text: "open AppCoordinator.swift",
            capabilities: VSCodeAdapter().capabilities,
            workspaceRoots: ["file://\(home)/Projects/opentypeless/OpenTypeless/Services/AppCoordinator.swift"]
        )
    ...
    @Test func windsurfDerivationAndRewriteFromSubdirectory() async {
        ...
        let result = await sut.rewrite(
            text: "open AppCoordinator.swift",
            capabilities: WindsurfAdapter().capabilities,
            workspaceRoots: ["\(home)/Projects/opentypeless/OpenTypeless/Services"]
        )
    ...
    @Test func allEditorsPrefixDeterminism() async {
        ...
        for (name, caps, expectedPrefix) in editors {
            ...
            #expect(
                result.text.contains("\(expectedPrefix)OpenTypeless/Services/AppCoordinator.swift"),
