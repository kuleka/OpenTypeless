...
struct PromptRoutingSignalTests {
    ...
    @Test func buildSignalFromSnapshotWithUnknownApp() {
        let appContext = AppContextInfo(
            ...
            selectedText: nil,
            documentPath: nil,
            browserURL: "https://github.com/kuleka/OpenTypeless"
        )
        let snapshot = ContextSnapshot(timestamp: Date(), appContext: appContext, clipboardText: nil, warnings: [])
    ...
    @Test func buildSignalUsesDirectoryDocumentPathAsWorkspaceRoot() {
        let appContext = AppContextInfo(
            ...
            focusedElementValue: nil,
            selectedText: nil,
            documentPath: "~/Projects/personal/opentypeless/",
            browserURL: nil
        )
        let snapshot = ContextSnapshot(timestamp: Date(), appContext: appContext, clipboardText: nil, warnings: [])
        ...
        #expect(signal.workspacePath == "~/Projects/personal/opentypeless")
    }

    @Test func buildSignalUsesExistingDirectoryPathWithoutTrailingSlash() throws {
        let tempDirectory = FileManager.default.temporaryDirectory
            .appendingPathComponent("opentypeless-routing-\(UUID().uuidString)", isDirectory: true)
    ...
    @Test func buildSignalDerivesWorkspaceFromTerminalWindowTitleWhenDocumentPathMissing() {
        let appContext = AppContextInfo(
            bundleIdentifier: "com.mitchellh.ghostty",
            appName: "Ghostty",
            windowTitle: "dev@machine:~/Projects/personal/opentypeless/",
            focusedElementRole: nil,
            focusedElementValue: nil,
            selectedText: nil,
            documentPath: nil,
    ...
    @Test func buildSignalDerivesWorkspaceFromTerminalFocusedValueWhenWindowTitleMissing() {
        let appContext = AppContextInfo(
            ...
            focusedElementValue: "dev@machine:~/Projects/personal/opentypeless/",
            selectedText: nil,
            documentPath: nil,
            browserURL: nil
        )
    ...
    @Test func buildSignalDoesNotUseTitlePathFallbackForNonTerminalApps() {
        let appContext = AppContextInfo(
            bundleIdentifier: "com.apple.Safari",
            appName: "Safari",
            windowTitle: "notes at ~/Projects/personal/opentypeless/",
            focusedElementRole: nil,
            focusedElementValue: nil,
            selectedText: nil,
            documentPath: nil,
