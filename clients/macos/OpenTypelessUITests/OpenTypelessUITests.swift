...
final class OpenTypelessUITests: XCTestCase {
    private let targetBundleIdentifier = "com.yisu.opentypeless"
    private let testModeKey = "OPENTYPELESS_TEST_MODE"
    private let uiTestModeKey = "OPENTYPELESS_UI_TEST_MODE"
    private let uiTestSurfaceKey = "OPENTYPELESS_UI_TEST_SURFACE"
    private let settingsTabKey = "OPENTYPELESS_UI_TEST_SETTINGS_TAB"
    private let settingsSearchTextKey = "OPENTYPELESS_UI_TEST_SETTINGS_SEARCH_TEXT"
    private let defaultsSuiteKey = "OPENTYPELESS_TEST_USER_DEFAULTS_SUITE"
    ...
    private func configuredApplication(
        ...
        let resolvedDefaultsSuite = defaultsSuite ?? "com.yisu.opentypeless.ui-tests.\(UUID().uuidString)"
    ...
    private func skipIfTargetAppIsAlreadyRunning() throws {
        ...
        if !runningApplications.isEmpty {
            throw XCTSkip("Quit OpenTypeless before running UI tests so XCTest does not force-terminate your active app session.")
        }
    }
    ...
    private func assertTargetAppIsNotAlreadyRunning() -> Bool {
        ...
        guard runningApplications.isEmpty else {
            XCTFail("Quit OpenTypeless before running UI tests so XCTest does not force-terminate your active app session.")
            return false
        }
        return true
    }
